...
import { DownloadIcon, ErrorIcon, FileIcon, ResetIcon, SuccessIcon, ChevronDownIcon, ChevronUpIcon } from './components/icons';
...
const App: React.FC = () => {
    ...
    const handleFileDrop = useCallback(async (file: File) => {
        ...
        try {
            const processedBlob = await processGltf(file, addLog);
            ...
            case 'processing':
                return (
                    <div className="flex flex-col items-center justify-center h-full text-center p-8">
                        ...
                        <p className="text-gray-400 mt-2">Applying gamma correction...</p>
                    </div>
                );
            case 'success':
                return (
                    ...
                    {logs.length > 0 && (
                        <>
                            <div className="p-4 border-t border-gray-700 flex items-center justify-between">
                                ...
                                <button onClick={() => setIsLogVisible(!isLogVisible)} className="p-1 rounded-full hover:bg-gray-700 transition-colors text-gray-400 hover:text-white">
                                    {isLogVisible ? <ChevronUpIcon className="w-6 h-6"/> : <ChevronDownIcon className="w-6 h-6"/>}
                                </button>
                            </div>
                            <div className={`transition-all duration-300 ${isLogVisible ? 'max-h-64' : 'max-h-0'} overflow-hidden`}>
